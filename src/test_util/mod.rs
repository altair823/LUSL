use crate::hash::make_md5_from_vec;
use std::{
    collections::HashMap,
    io,
    path::{Path, PathBuf},
...
pub fn get_original_md5() -> HashMap<String, String> {
    make_md5_from_vec(get_file_list(ORIGINAL_DIR).unwrap())
}

pub fn get_result_md5<O: AsRef<Path>>(dir: O) -> HashMap<String, String> {
    make_md5_from_vec(get_file_list(dir).unwrap())
}
